...
let lastFoodSpawn = 0;
let pendingGrowth = 0;
let score = localStorage.getItem('reverseScore') ? parseInt(localStorage.getItem('reverseScore')) : 0;
let highScore = localStorage.getItem('reverseHigh') ? parseInt(localStorage.getItem('reverseHigh')) : 0;
...
function resetGame(clearScores = false) {
    const startX = Math.floor(Math.random() * (tileCountX || 40));
    const startY = Math.floor(Math.random() * (tileCountY || 30));
    ...
    if (clearScores) {
        highScore = 0;
        localStorage.setItem('reverseScore', '0');
        localStorage.setItem('reverseHigh', '0');
    }
...
function update() {
    ...
    if (foodIdx !== -1) {
        ...
        pendingGrowth -= 1; // Remove segment instead of add
        scoreDisplay.textContent = score;
    }
    ...
    if (goldenFood && head.x === goldenFood.x && head.y === goldenFood.y) {
        score += 50;
        goldenFood = null;
        pendingGrowth -= 5; // Remove segments
        scoreDisplay.textContent = score;
    }
    ...
    if (diamondFood && head.x === diamondFood.x && head.y === diamondFood.y) {
        score += 100;
        diamondFood = null;
        pendingGrowth -= 10; // Remove segments
        scoreDisplay.textContent = score;
    }
    ...
    if (sapphireFood && head.x === sapphireFood.x && head.y === sapphireFood.y) {
        score += 250;
        sapphireFood = null;
        pendingGrowth -= 25; // Remove segments
        scoreDisplay.textContent = score;
    }
    ...
    if (bronzeFood && head.x === bronzeFood.x && head.y === bronzeFood.y) {
        score += 30;
        bronzeFood = null;
        pendingGrowth -= 3; // Remove segments
        scoreDisplay.textContent = score;
    }
    ...
    if (pendingGrowth > 0) {
        ...
        // Remove segment in reverse mode
        if (snake.length > 1) {
            snake.pop();
        }
        pendingGrowth += 1;
        
        // End game if snake reaches length 0
        if (snake.length === 0) {
            endGame();
            return;
        }
...
function endGame() {
    ...
    if (score > highScore) {
        highScore = score;
        localStorage.setItem('reverseHigh', highScore);
        highScoreDisplay.textContent = highScore;
    }
    localStorage.setItem('reverseScore', score);
