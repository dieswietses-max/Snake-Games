...
let lastFoodSpawn = 0;
let lastWallSpawn = 0;
const wallSpawnInterval = 5000; // New wall every 5 seconds
let pendingGrowth = 0;
let score = localStorage.getItem('mazeRunnerScore') ? parseInt(localStorage.getItem('mazeRunnerScore')) : 0;
let highScore = localStorage.getItem('mazeRunnerHigh') ? parseInt(localStorage.getItem('mazeRunnerHigh')) : 0;
...
function resetGame(clearScores = false) {
    ...
    if (clearScores) {
        highScore = 0;
        localStorage.setItem('mazeRunnerScore', '0');
        localStorage.setItem('mazeRunnerHigh', '0');
    }
...
function isOccupied(x, y) {
    const occupied = [...snake, ...foods, ...walls];
...
function generateWalls() {
    for (let i = 0; i < 3; i++) {
        ...
        do {
            ...
            attempts += 1;
            if (!isOccupied(newWall.x, newWall.y)) {
                walls.push(newWall);
                break;
            }
        } while (attempts < 20);
...
function update() {
    ...
    const wallSpawnInterval = Math.max(2000, 5000 - (score * 20)); // Min 2 seconds, starts at 5 seconds, decreases with score
    if (now - lastWallSpawn >= wallSpawnInterval) {
        generateWalls();
        lastWallSpawn = now;
    }
...
function endGame() {
    ...
    if (score > highScore) {
        highScore = score;
        localStorage.setItem('mazeRunnerHigh', highScore);
        highScoreDisplay.textContent = highScore;
    }
    localStorage.setItem('mazeRunnerScore', score);
