...
// Game constants
const gridSize = 20;
const maxTotalFood = 40;
const normalFoodSpawnCooldown = 200; // Very fast spawning
const normalFoodSpawnChance = 0.8; // High spawn chance
const goldenSpawnChance = 0.3;
const diamondSpawnChance = 0.15;
const sapphireSpawnChance = 0.05;
const bronzeSpawnChance = 0.6;
...
let gameStartTime = 0;
let score = localStorage.getItem('timeAttackScore') ? parseInt(localStorage.getItem('timeAttackScore')) : 0;
let highScore = localStorage.getItem('timeAttackHigh') ? parseInt(localStorage.getItem('timeAttackHigh')) : 0;
let gameRunning = false;
let gameOver = false;
let gamePaused = false;
let gameSpeed = 120; // Faster initial speed

scoreDisplay.textContent = score;
highScoreDisplay.textContent = highScore;
timeLeftDisplay.textContent = gameTimeLimit;
...
function resetGame(clearScores = false) {
    ...
    if (clearScores) {
        highScore = 0;
        localStorage.setItem('timeAttackScore', '0');
        localStorage.setItem('timeAttackHigh', '0');
    }
...
function update() {
    ...
    timeRemaining = Math.max(0, gameTimeLimit - elapsed);
...
function endGame() {
    ...
    if (score > highScore) {
        highScore = score;
        localStorage.setItem('timeAttackHigh', highScore);
        highScoreDisplay.textContent = highScore;
    }
    localStorage.setItem('timeAttackScore', score);
