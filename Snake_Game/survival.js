...
let lastFoodSpawn = 0;
let pendingGrowth = 0;
let score = localStorage.getItem('survivalScore') ? parseInt(localStorage.getItem('survivalScore')) : 0;
let highScore = localStorage.getItem('survivalHigh') ? parseInt(localStorage.getItem('survivalHigh')) : 0;
...
function resetGame(clearScores = false) {
    ...
    if (clearScores) {
        highScore = 0;
        localStorage.setItem('survivalScore', '0');
        localStorage.setItem('survivalHigh', '0');
    }
...
function update() {
    ...
    // Check if out of bounds (no wrapping)
...
function endGame() {
    ...
    if (score > highScore) {
        highScore = score;
        localStorage.setItem('survivalHigh', highScore);
        highScoreDisplay.textContent = highScore;
    }
