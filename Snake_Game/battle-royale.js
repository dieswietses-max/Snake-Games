...
let lastFoodSpawn = 0;
let pendingGrowth = 0;
let score = localStorage.getItem('battleRoyaleScore') ? parseInt(localStorage.getItem('battleRoyaleScore')) : 0;
let highScore = localStorage.getItem('battleRoyaleHigh') ? parseInt(localStorage.getItem('battleRoyaleHigh')) : 0;
...
function resetGame(clearScores = false) {
    ...
    if (clearScores) {
        highScore = 0;
        localStorage.setItem('battleRoyaleScore', '0');
        localStorage.setItem('battleRoyaleHigh', '0');
    }
...
function update() {
    ...
    // Shrink zone every 30 seconds
    if (now - lastShrink >= shrinkInterval) {
        shrinkZone += 1;
        lastShrink = now;
    }
...
function draw() {
    ...
    // Draw death zone (red border)
    if (shrinkZone > 0) {
        ctx.fillStyle = 'rgba(255, 0, 0, 0.3)';
        // Top
        ctx.fillRect(0, 0, canvas.width, shrinkZone * gridSize);
        // Bottom
        ctx.fillRect(0, (tileCountY - shrinkZone) * gridSize, canvas.width, shrinkZone * gridSize);
        // Left
        ctx.fillRect(0, shrinkZone * gridSize, shrinkZone * gridSize, (tileCountY - 2 * shrinkZone) * gridSize);
        // Right
        ctx.fillRect((tileCountX - shrinkZone) * gridSize, shrinkZone * gridSize, shrinkZone * gridSize, (tileCountY - 2 * shrinkZone) * gridSize);
    }
...
function endGame() {
    ...
    if (score > highScore) {
        highScore = score;
        localStorage.setItem('battleRoyaleHigh', highScore);
        highScoreDisplay.textContent = highScore;
    }
    localStorage.setItem('battleRoyaleScore', score);
