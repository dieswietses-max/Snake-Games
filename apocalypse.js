...
const enemySpawnInterval = 15000; // Spawn enemy every 15 seconds
const enemyMoveSpeed = 300; // Enemy moves every 300ms
...
let lastEnemyMove = 0;
let lastEnemyDeath = 0; // Track when an enemy died for respawn cooldown
let enemyDeathCooldown = 10000; // 10 seconds before spawning new enemy after death
let pendingGrowth = 0;
let score = localStorage.getItem('apocalypseSnakeScore') ? parseInt(localStorage.getItem('apocalypseSnakeScore')) : 0;
let highScore = localStorage.getItem('apocalypseSnakeHigh') ? parseInt(localStorage.getItem('apocalypseSnakeHigh')) : 0;
...
enemyCountDisplay.textContent = 0;
...
function wrapEntitiesToBounds() {
    ...
    enemySnakes.forEach(enemy => {
        enemy.body = enemy.body.map(wrapPoint);
    });
...
function resetGame(clearScores = false) {
    ...
    if (clearScores) {
        highScore = 0;
        localStorage.setItem('apocalypseSnakeScore', '0');
        localStorage.setItem('apocalypseSnakeHigh', '0');
    }
...
function isOccupied(x, y) {
    ...
    enemySnakes.forEach(enemy => occupied.push(...enemy.body));
...
function spawnEnemySnake() {
    ...
    enemyCountDisplay.textContent = enemySnakes.length;
}
...
function moveEnemies() {
    enemySnakes.forEach(enemy => {
        ...
        if (Math.abs(dx) > Math.abs(dy)) {
            enemy.direction = dx > 0 ? { x: 1, y: 0 } : { x: -1, y: 0 };
        } else {
            enemy.direction = dy > 0 ? { x: 0, y: 1 } : { x: 0, y: -1 };
        }
        
        // Move enemy
        const newX = (head.x + enemy.direction.x + tileCountX) % tileCountX;
        const newY = (head.y + enemy.direction.y + tileCountY) % tileCountY;
        enemy.body.unshift({ x: newX, y: newY });
        enemy.body.pop();
    });
}
...
function update() {
    ...
    // Spawn enemy after 10 second cooldown if one died
    if (lastEnemyDeath > 0 && now - lastEnemyDeath > enemyDeathCooldown) {
        spawnEnemySnake();
        lastEnemyDeath = 0; // Reset death timer
    }
    
    // Spawn enemy every 100 points
    ...
    // Move enemies at different speed (scales with score)
    const currentEnemySpeed = Math.max(100, enemyMoveSpeed - Math.floor(score / 10));
    if (now - lastEnemyMove >= currentEnemySpeed) {
        moveEnemies();
        lastEnemyMove = now;
    }
    ...
    // Check collision with enemy snakes and remove enemies that hit the player
    for (let i = enemySnakes.length - 1; i >= 0; i--) {
        const enemy = enemySnakes[i];
        const enemyHead = enemy.body[0];
        
        // Check if enemy head hits player snake
        for (let segment of snake) {
            if (enemyHead.x === segment.x && enemyHead.y === segment.y) {
                enemySnakes.splice(i, 1);
                enemyCountDisplay.textContent = enemySnakes.length;
                lastEnemyDeath = now; // Mark time of death for respawn cooldown
                break;
            }
        }
        
        // Check if player head hits enemy body
        if (i < enemySnakes.length) {
...
function endGame() {
    ...
    if (score > highScore) {
        highScore = score;
        localStorage.setItem('apocalypseSnakeHigh', highScore);
        highScoreDisplay.textContent = highScore;
    }
    localStorage.setItem('apocalypseSnakeScore', score);
